...
# Add paths (same as your code)
CURRENT_FILE = os.path.abspath(__file__)
VALIDATIONS_DIR = os.path.dirname(CURRENT_FILE)
PROJECT_ROOT = os.path.dirname(VALIDATIONS_DIR)
...
def reproduce_care_table7(n_samples=50, n_cf=10):
    ...
    task = 'classification'
    blackbox_name = 'gb-c'
    blackbox = CreateModel(dataset, X_train, X_test, Y_train, Y_test, task, blackbox_name, GradientBoostingClassifier)
    ...
    for x_ord in X_test:
        try:
            ...
            cfs_ord_config_1, cfs_eval_config_1, x_cfs_ord_config_1, x_cfs_eval_config_1 = \
                evaluateCounterfactuals(x_ord, explanation_config_1['cfs_ord'], dataset,
            ...
            cfs_ord_config_12, cfs_eval_config_12, x_cfs_ord_config_12, x_cfs_eval_config_12 = \
                evaluateCounterfactuals(x_ord, explanation_config_12['cfs_ord'], dataset,
                                       predict_fn, predict_proba_fn, task, toolbox,
                                       objective_names, featureScaler, feature_names)
            ...
            cfs_ord_config_123, cfs_eval_config_123, x_cfs_ord_config_123, x_cfs_eval_config_123 = \
                evaluateCounterfactuals(x_ord, explanation_config_123['cfs_ord'], dataset,
                                       predict_fn, predict_proba_fn, task, toolbox,
                                       objective_names, featureScaler, feature_names)
            ...
            cfs_ord_config_1234, cfs_eval_config_1234, x_cfs_ord_config_1234, x_cfs_eval_config_1234 = \
                evaluateCounterfactuals(x_ord, explanation_config_1234['cfs_ord'], dataset,
                                       predict_fn, predict_proba_fn, task, toolbox,
                                       objective_names, featureScaler, feature_names)
    ...
    # The columns from evaluateCounterfactuals are:
    # Outcome, Actionability, Coherency, Proximity, Connectedness, Distance, Sparsity, i-Validity, s-Validity, f-Diversity, v-Diversity, d-Diversity
    metric_names = ['Outcome', 'Actionability', 'Coherency', 'Proximity', 'Connectedness', 'Distance', 'Sparsity']
    ...
    for config_name, config_results in [
        ...
        if len(config_results) > 0:
            ...
            for i, metric in enumerate(metric_names):
                results[config_name][metric.lower()] = {
                    'mean': means[i],
                    'std': stds[i]
                }
